...
make_EHelper(sub) {
  ...
  // update CF (dest,res,src1)
  rtl_is_sub_carry(&s1, &s0, &id_dest->val);//可能有问题
  rtl_set_CF(&s1);

  // update OF(dest,res,src1,src2)
...
make_EHelper(cmp) {
  // s0 = dest - src = result
  rtl_sub(&s0, &id_dest->val, &id_src->val);
  //printf("s0=dest-src:s0:%d,dest:%d,src:%d\n",s0,id_dest->val,id_src->val);
